...
import { Truck, Info, DollarSign, MapPin, Image as ImageIcon, Upload, X, ArrowLeft, FileText, Weight } from 'lucide-react';
...
export default function EditVehiclePage({ params }: { params: Promise<{ id: string }> }) {
        ...
        if (res.ok) {
          ...
          setFormData({
            ...
            firstRegistrationDate: vehicle.firstRegistrationDate
              ? new Date(vehicle.firstRegistrationDate).toISOString().split('T')[0]
              : '',
            trailerRegistrationNumber: vehicle.trailerRegistrationNumber || '',
            color: vehicle.color || '',
            capacityWeight: vehicle.capacityWeight?.toString() || '',
            capacityVolume: vehicle.capacityVolume?.toString() || '',
            isOffRoadCapable: vehicle.isOffRoadCapable,
            hasDriver: vehicle.hasDriver,
            hasConvoyeur: vehicle.hasConvoyeur || false,
            fuelType: vehicle.fuelType || 'Diesel',
            transmission: vehicle.transmission || 'Manuelle',
            location: vehicle.location,
            pricingType: vehicle.pricingType || 'PER_TONNE_KM',
            pricePerDay: vehicle.pricePerDay?.toString() || '',
            pricePerKm: vehicle.pricePerKm?.toString() || '',
            pricePerTonneKm: vehicle.pricePerTonneKm?.toString() || '',
            minPrice: vehicle.minPrice?.toString() || '',
            maxPrice: vehicle.maxPrice?.toString() || '',
            conditions: vehicle.conditions || '',
            assuranceDocUrl: vehicle.assuranceDocUrl || '',
            visiteTechniqueDocUrl: vehicle.visiteTechniqueDocUrl || '',
            carteGriseDocUrl: vehicle.carteGriseDocUrl || '',
            patenteDocUrl: vehicle.patenteDocUrl || '',
            photoFrontUrl: vehicle.photoFrontUrl || '',
            photoRearUrl: vehicle.photoRearUrl || '',
            photoLeftUrl: vehicle.photoLeftUrl || '',
            photoRightUrl: vehicle.photoRightUrl || '',
            images: vehicle.images?.map(img => img.url) || (vehicle.imageUrl ? [vehicle.imageUrl] : []),
          });
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/vehicles/${id}`, {
        ...
        body: JSON.stringify({
          ...formData,
          capacityWeight: formData.capacityWeight ? parseFloat(formData.capacityWeight) : null,
          capacityVolume: formData.capacityVolume ? parseFloat(formData.capacityVolume) : null,
          ...
          pricePerTonneKm: formData.pricePerTonneKm ? parseFloat(formData.pricePerTonneKm) : null,
  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900">Modifier le véhicule</h1>
        <p className="text-slate-500 mt-2">Modifiez les informations de votre véhicule.</p>
      </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-3">
              {[
                { value: 'PER_TONNE_KM', label: 'Par Tonne-Km' },
                { value: 'PER_DAY', label: 'Par Jour' },
                { value: 'PER_KM', label: 'Par Kilomètre' },
              ].map(({ value, label }) => (
                <button
                  key={value}
                  type="button"
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-600 mt-1">
                    Prix total = <strong>Tonnage × Distance (km) × Tarif</strong>.<br/>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            {PHOTO_SIDES.map(({ key, label }) => {
              const url = formData[key as SideKey];
              return (
                  ...
                  {url ? (
                    ...
                    <label className="flex flex-col items-center justify-center aspect-video border-2 border-dashed border-slate-300 rounded-lg cursor-pointer hover:border-blue-500 hover:bg-blue-50 transition">
                      ...
                      <input
                        ...
                        className="hidden"
                        accept="image/*"
                        onChange={(e) => handleSidePhotoUpload(e, key as SideKey)}
          ...
          <p className="text-sm text-slate-500 mb-3">Photos supplémentaires</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {DOCS.map(({ key, label, required }) => {
              const url = formData[key as DocKey];
              return (
                <div key={key}>
                  ...
                  {url ? (
                    ...
                    <label className={`flex items-center gap-3 p-3 border-2 border-dashed rounded-lg cursor-pointer hover:bg-blue-50 transition ${uploading === key ? 'border-blue-300' : 'border-slate-300 hover:border-blue-400'}`}>
                      ...
                      <input
                        ...
                        className="hidden"
                        accept="image/*,.pdf"
                        onChange={(e) => handleDocUpload(e, key as DocKey)}
                        disabled={uploading !== null}
                      />
                    </label>
                  )}
